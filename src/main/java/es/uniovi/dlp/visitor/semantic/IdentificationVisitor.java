...
public class IdentificationVisitor extends AbstractVisitor<Type, Type>  {
    ...
    public Type visit(Struct struct, Type param) {
        ...
        struct.getFields().stream().forEach(field -> {
            if (!fields.contains(field.getName())) {
                fields.add(field.getName());
            } else {
                ErrorManager.getInstance().addError(new Error(new Location(field.getLine(), field.getColumn()), ErrorReason.FIELD_ALREADY_DECLARED));
            }
        });
        return null;
    }
    ...
    public Type visit(VarDefinition varDefinition, Type param) {
        if (st.findInCurrentScope(varDefinition.getName()) == null) {
            ...
            ErrorManager.getInstance().addError(new Error(new Location(varDefinition.getLine(), varDefinition.getColumn()), ErrorReason.VARIABLE_ALREADY_DECLARED));
        }
    ...
    public Type visit(FunctionDefinition functionDefinition, Type param) {
        if (st.find(functionDefinition.getName()) == null) {
            ...
            ErrorManager.getInstance().addError(new Error(new Location(functionDefinition.getLine(), functionDefinition.getColumn()), ErrorReason.FUNCTION_ALREADY_DECLARED));
        }
    ...
    public Type visit(Invocation invocation, Type param) {
        if (st.find(invocation.getName().getName()) == null)
            ErrorManager.getInstance().addError(new Error(new Location(invocation.getLine(), invocation.getColumn()), ErrorReason.FUNCTION_NOT_DECLARED));
        else
    ...
    public Type visit(Variable variable, Type param) {
        if (st.find(variable.getName()) == null) {
            ErrorManager.getInstance().addError(new Error(new Location(variable.getLine(), variable.getColumn()), ErrorReason.VARIABLE_NOT_DECLARED));
            variable.setDefinition(new VarDefinition("Error", new ErrorType(variable.getLine(), variable.getColumn()), variable.getLine(), variable.getColumn()));
        } else {
